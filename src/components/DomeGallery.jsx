...
function buildItems(pool, seg) {
  ...
  if (pool.length > totalSlots) {
    console.warn(
      `[DomeGallery] Provided skill count (${pool.length}) exceeds available tiles (${totalSlots}). Some skills will not be shown.`
    );
  }

  const normalizedSkills = pool.map((skill) => {
    return {
  ...
  const usedSkills = Array.from(
    { length: totalSlots },
    (_, i) => normalizedSkills[i % normalizedSkills.length]
  );

  // Avoid duplicates next to each other
  for (let i = 1; i < usedSkills.length; i++) {
    if (usedSkills[i].name === usedSkills[i - 1].name) {
      for (let j = i + 1; j < usedSkills.length; j++) {
        if (usedSkills[j].name !== usedSkills[i].name) {
          const tmp = usedSkills[i];
          usedSkills[i] = usedSkills[j];
          usedSkills[j] = tmp;
          break;
        }
      }
...
export default function DomeGallery({
  ...
  grayscale = false,
  iconSize = 80,
  tileBackground = 'bg-gradient-to-br from-gray-800 to-gray-900',
}) {
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const originalIconContainer = overlay.querySelector('.icon-container');
      if (originalIconContainer) {
        const iconContainer = originalIconContainer.cloneNode(true);
        animatingOverlay.appendChild(iconContainer);
      }
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`item__image absolute block overflow-hidden cursor-pointer ${tileBackground} transition-transform duration-300`}
                      role="button"
                      tabIndex={0}
                      aria-label={it.name || 'Open skill'}
                      onClick={(e) => {
